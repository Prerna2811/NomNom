...
# Set Google API key
os.environ['GOOGLE_API_KEY'] = "AIzaSyCHv8wP57YpN54CrR9TSo4gpK0RYoA0RbM"
...
tru_app = TruCustomApp(food_app, app_id = 'Food App')
    ...
    if uploaded_image is not None:
        ...
        if st.markdown(button_html, unsafe_allow_html=True):
            ...
            while not isinstance(recipe_data, dict):
                recipe_data_str = food_app.generate_recipe_data(image, serving_size)
            ...
            if type(recipe_data)==dict:
                with st.container():
                    ...
                    with tru_app as recording:
                        food_app.generate_recipe_data(image,serving_size)
                        records, feedback = tru.get_records_and_feedback(app_ids=["Food App"])
                        print("------")
                        print(type(records))
